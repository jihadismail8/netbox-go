...
/**
 * InterfaceTypeChoices from the pinned NetBox v4.4.6-post7 compatibility
 * baseline. Keep this closed list aligned with the backend domain validator so
 * the browser never hides a valid baseline interface type or submits an
 * extension value by accident.
 */
const interfaceTypeValues = `
virtual bridge lag
...
export const INTERFACE_TYPE_CHOICES: ChoiceOption[] = interfaceTypeValues.map((value) => ({
  value,
  label: friendlyLabels[value] ?? value.toUpperCase(),
}))
