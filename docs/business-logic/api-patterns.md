# API serialization patterns and custom-endpoint discovery

> **Partial historical extraction, not an accepted API specification.** This file does not reconcile to the authoritative 23-action inventory and omits entire modules. Bulk examples describe upstream capability classes, but bulk operations are deferred from `core-workflow-v1`. Verify every route, shape, permission, transaction, and side effect against the pinned source, accepted profile, and strict oracle before implementation.
