# DCIM models — partial business-logic discovery

> Historical extraction from selected Python NetBox `dcim/models/` files, not an accepted specification. It contains known relationship/delete inaccuracies. Reconcile each rule to the pinned source, upstream tests, Capability Profile, and oracle before implementation.
