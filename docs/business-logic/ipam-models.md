# IPAM Models — Business Logic Reference

> Extracted from Python NetBox `ipam/models/` for porting to Go.
> **Source:** `netbox/netbox/ipam/models/{vrfs,ip,vlans,fhrp,services,asns}.py`
...
**clean() rules:**
0. **Public API prefix validation** — `prefix_validator` rejects host-bit input and returns the canonical CIDR as a suggestion before model save.
...
## Summary: Key IPAM Business Rules for Go Port
