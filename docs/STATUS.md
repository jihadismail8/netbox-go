...
bearer parsing, and fail-closed unary method safety. `CW1-V1-02-I3` is active
for typed browser-session outcomes, valid-session-first REST arbitration,
exact CSRF pairing/recovery, transactional login/logout, and cookie shape. Its
parent and the password-session, throttle, trusted-proxy, streaming, and
aggregate rows remain open. The 293-row
...
| V1 identity/RBAC         | Persisted identity, retained CORS, accepted bounded token lookup/touch and token-transport evidence | Complete active I3, then password/throttle/proxy/streaming plus RBAC/admin/secret work                          | V1-01/I1/I2 done; I3 active           |
