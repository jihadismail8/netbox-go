...
`CW1-V2-02-I8` is the active reviewed increment for InterfaceTemplate
`device_type`, `name`, `label`, `type`, `enabled`, `mgmt_only`, and
`description` common POST/PUT/PATCH presence across generated contracts,
typed REST/gRPC/application semantics, PostgreSQL, and Vue. It claims no
implementation evidence yet. It exercises existing DeviceType-owner
...
non-retroactivity, deletion, list/query behavior, full CRUD, every tier and
