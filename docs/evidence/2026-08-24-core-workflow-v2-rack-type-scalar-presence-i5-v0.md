# Core Workflow RackType scalar write presence — 2026-08-24
...
- State: `evidence`; effective at exact claim revision
  `25d6f10bab78b9b92c1682fe9b951cc4f2286ea8` after repository CI.
...
This artifact proves only RackType `manufacturer`, `model`, `slug`,
`form_factor`, `width`, `u_height`, `starting_unit`, `desc_units`,
`description`, and `comments` create/PUT/PATCH presence, defaults, validation,
numeric-ID Manufacturer input, generated contracts, shared REST/gRPC
semantics, PostgreSQL durability, Vue handling, and the eight packet tests
specified by the [`CW1-V2-02-I5` packet](../increments/CW1-V2-02-I5.md).

It does not complete RackType CRUD, uniqueness, Rack propagation, alternate
...
- Entry revision:
  `9f1ef5697f0e1457ee2e7574977444b8f2356bbe`.
- Entry source digest:
  `source-v2:sha256:226380408886732e2ca50dbee8b9b456e4e944c4427eb38518b3b8bab9ad523d`
  with 3,036 entries.
- Reviewed packet revision:
  `c39a8ad48bb696984bb02bab11bca7a0ecd4261f`.
- Tested source candidate:
  `89507d95d2743de7f97d64ca14cc43f6b834770b`.
- Tested source digest:
  `source-v2:sha256:a8325eaae703aa801ed587deefae7e8d08d9c9e0189c80ff7569da95c36d6f90`.
- Tested owned entries: 3,043.
- Canonical source-manifest output SHA-256:
  `eb1606e0d677e76ae806a73f8846a7a9bd90bfbc3cf8914e0b75e783ead7d55c`.
- Evidence-claim source digest: unchanged from the tested source digest, with
  the same 3,043 owned entries.
- Evidence-claim source-manifest output SHA-256: unchanged at
  `eb1606e0d677e76ae806a73f8846a7a9bd90bfbc3cf8914e0b75e783ead7d55c`.
- Complete derived source-manifest diff SHA-256:
  `0885bd65790ca52f2d86b7c52bd0c6da711b424cff3f056b2a6c605e7efd2e21`.
...
- Operation-specific RackType metadata generates distinct `RackTypeCreate`,
  `RackTypeReplace`, and `RackTypeUpdate` request schemas. Create requires
  `manufacturer`, `model`, `slug`, and `form_factor`; replace requires only
  `manufacturer`, `model`, and `slug`; update requires no field. Manufacturer
  request input is a non-null integer ID, response Manufacturer is a non-null
  `ObjectReference`, and only the two response timestamps are nullable.
- POST requires Manufacturer, model, slug, and a valid form factor; omitted
  form factor still reaches and fails the pinned nonblank model check. It
  defaults width to `19`, height to `42`, starting unit to `1`, descending
  units to `false`, and description/comments to `""`. PUT preserves omitted
  form factor and every optional scalar. PATCH preserves every omitted scalar.
- Explicit null is rejected for all ten fields. Model, slug, and form factor
  reject blank; description and comments accept blank clears. Form factor is
  intentionally not trimmed and accepts only the seven pinned choices. Width
  accepts only `10`, `19`, `21`, or `23`; height is `1..100`; starting unit is
  `1..32767`; concrete zero and `false` survive transport decoding until the
  typed contract validates or persists them.
- The typed command/domain boundary preserves all operation-specific presence
  states, validates once in deterministic field order without discarding
...
- Every public mutation uses one application-owned transaction containing
  ...
  the pre-existing untouched propagation call, and one RackType object-change
  record. Validation rejection causes zero repository mutation or propagation
  calls. Repository and recorder failures roll back durable scalar state,
  timestamps, and object changes.
- REST retains omitted, null, blank, zero, false, and concrete scalar intent.
  Supported gRPC FieldMask paths with absent optional values become explicit
  null intent; unknown paths fail closed. Optional protobuf scalars still
  cannot independently encode POST/PUT null, and no protobuf was regenerated.
- Real PostgreSQL reloads all ten raw scalar values and proves timestamp,
  RackType/global row, and object-change counts plus rollback. The fixtures
  contain zero referencing Rack rows and assert zero Rack changes, so this
  increment neither exercises nor accepts propagation semantics.
- Vue retains nullable response timestamps, numeric Manufacturer-ID mapping,
  required/default/choice/range metadata, false/zero/blank/concrete values,
  and a private normalized edit baseline that omits unchanged PATCH fields.
- Traceability adds only the unresolved RackType scalar-presence assessment
  and remaps create/replace/update. The resource-contract row remains on its
  pre-existing contradicted filter-name assessment.

RackType uniqueness, alternate Manufacturer input grammar, delete protection,
list/get/delete/filter/search/order/pagination behavior, permissions beyond
the authorized fixtures, empty-PATCH semantics, propagation, full CRUD, and
all other resources remain outside this increment.

## Changed tested-source files

The tested candidate differs from the accepted I4 entry revision across
exactly 28 regular mode-`100644` paths:
...
4. `docs/ROADMAP.md`
5. `docs/STATUS.md`
6. `docs/increments/CW1-V2-02-I5.md` — new
7. `docs/increments/README.md`
8. `netbox-backend/api/openapi/netbox-go-v1.yaml`
9. `netbox-backend/internal/adapters/grpc/workflow/rack_type_typed_handler.go`
10. `netbox-backend/internal/adapters/grpc/workflow/rack_type_typed_handler_test.go`
11. `netbox-backend/internal/adapters/grpc/workflow/rack_type_typed_mapper_test.go` — new
12. `netbox-backend/internal/adapters/postgres/dcim/rack_type_service_real_postgres_test.go` — new
13. `netbox-backend/internal/adapters/rest/netbox/workflow/rack_type_scalar_presence_test.go` — new
14. `netbox-backend/internal/application/dcim/rack_type_commands.go`
15. `netbox-backend/internal/application/dcim/rack_type_commands_test.go` — new
16. `netbox-backend/internal/application/dcim/rack_type_service.go`
17. `netbox-backend/internal/application/dcim/rack_type_service_presence_test.go` — new
18. `netbox-backend/internal/domain/dcim/rack_type.go`
19. `netbox-backend/internal/domain/dcim/rack_type_test.go`
20. `netbox-backend/test/parity/rack_type_scalar_presence_test.go` — new
21. `netbox-frontend/src/features/core/adapters.test.ts`
22. `netbox-frontend/src/features/core/adapters.ts`
23. `netbox-frontend/src/features/core/resources.ts`
24. `netbox-frontend/src/router/models/core-profile.test.ts`
25. `netbox-frontend/src/router/models/core-profile.ts`
26. `scripts/validate_capability_profile.mjs`
27. `scripts/validate_capability_profile.test.mjs`
28. `scripts/validate_openapi.mjs`

The entry-to-candidate delta contains 3,538 insertions and 203 deletions,
including seven new regular files. No path was deleted, renamed, retargeted,
type-changed, mode-changed, or made executable.
...
TestRackTypeScalarValidationLeavesStateUnchanged
TestRackTypeRESTScalarPresenceMatrix
TestRackTypeGRPCScalarPresenceMatrix
TestPostgresRackTypeScalarPresenceDurability
TestRackTypeScalarPresenceRESTGRPCParity
```

They span the domain, application, REST, gRPC, PostgreSQL, and parity packages.
Independent semantic, adapter/PostgreSQL/parity, and generated-contract/Vue
reviews found no remaining blocker. Review first exposed an over-broad
blankable-choice validator classifier for non-choice numeric/boolean fields;
the classifier and adversarial `u_height`, `starting_unit`, and `desc_units`
tests were corrected. Review then exposed missing explicit REST PUT-omission
and cross-transport concrete PATCH proof; the parity matrix was expanded for
both directions before the exact candidate gates.
...
The final unchanged source digest passed the exact affected non-PostgreSQL
package set and its race-enabled replay. The parity package completed in
13.935 seconds without race instrumentation and 177.104 seconds with it.

The focused durable PostgreSQL test completed in 0.321 seconds without a skip
at `2026-08-24T21:17:26Z`. The final exact committed full L4 replay ran from
`2026-08-24T21:17:41Z` through `2026-08-24T21:18:16Z` and passed with these
package timings:

| Package                                | Result  |
| -------------------------------------- | ------: |
| `internal/database`                    |  2.537s |
| `internal/adapters/postgres/bootstrap` |  0.045s |
| `internal/adapters/postgres/dcim`      |  2.574s |
| `internal/adapters/postgres/dcim/row`  |  0.365s |
| `internal/adapters/postgres/ipam`      |  1.892s |
| `internal/adapters/postgres/ipam/row`  |  0.424s |
| `internal/adapters/postgres/changelog` |  0.010s |
| `internal/application/dcim`            |  0.010s |
| `internal/application/ipam`            |  0.073s |
| `internal/application/identity`        | 34.832s |
| `cmd/netbox_go_admin`                  |  6.845s |

The run used exact disposable database `netbox_i5_20260824` in the existing
test PostgreSQL container with the shared test owner; no disposable role was
created. No credential or complete DSN is retained. Before each final drop the
database had zero connections. It was dropped; final connection, database,
and same-named-role counts were `0|0|0`. The shared owner was untouched. The
disposable database is not recoverable.

The first sandbox-only full-backend run failed solely because frozen cache/DAO
tests could not bind `127.0.0.1:0` (`EPERM`). Sandbox-only Node/Git/esbuild and
PostgreSQL-socket attempts were likewise denied. The identical approved
isolated commands passed. These were environment-only failures, not behavioral
failures, and no gate was weakened.

## Generated contracts and frontend results

The final candidate passed:

- all 101 capability-profile and generated-OpenAPI adversarial tests;
...
- 35 focused Vue adapter/form tests and both production and test typechecks;
  and
- the complete frontend gate: 29 files, 155 tests, 63.2% statement coverage,
  formatting, lint, typechecking, and production build.
...
pinned toolchain path and task-local caches:
...
env \
  ...
  GOLANGCI_LINT_CACHE=/tmp/netbox-go-i5-rack-type-golangci-cache \
  npm_config_cache=/tmp/netbox-go-npm-cache \
  make check
```

It ran from `2026-08-24T21:19:04Z` through `2026-08-24T21:24:01Z` and exited
0. Results included:

- backend lint: zero issues;
- backend coverage: 12.7759% (37,218/291,313), above the retained 12.4001%
...
- every contract, traceability, frontend, and generated-output result above;
  and
- all local links valid across 167 Markdown files.
...
contract, frontend, backend, whole-repository, commit, and push boundaries.

## Independent candidate CI

- Workflow: `repository-gate`.
- Run: [32779345446](https://github.com/jihadismail8/netbox-go/actions/runs/32779345446).
- Job `check`: [97597697534](https://github.com/jihadismail8/netbox-go/actions/runs/32779345446/job/97597697534).
- Branch: `work/cw1-v2-02-i5-rack-type-presence`.
- Head SHA: `89507d95d2743de7f97d64ca14cc43f6b834770b`.
- Run start/completion: `2026-08-24T21:24:44Z`–`2026-08-24T21:44:32Z`.
- Job start/completion: `2026-08-24T21:24:47Z`–`2026-08-24T21:44:31Z`.
...
`89507d95d2743de7f97d64ca14cc43f6b834770b`, not a different merge revision.

## Digest-excluded claim local gate

The evidence-only worktree changes only the two source-v2-excluded paths
listed below. Its source digest, 3,043-entry manifest, and canonical manifest
...
env \
  ...
  GOCACHE=/tmp/go-cache \
  GOFLAGS=-buildvcs=false \
  GOLANGCI_LINT_CACHE=/tmp/netbox-go-i5-evidence-golangci-cache \
  npm_config_cache=/tmp/netbox-go-npm-cache \
  make check
```

It exited 0 and retained backend coverage at 12.7759% (37,218/291,313), 54
measured packages and one reviewed exclusion, all 29 frontend files and 155
tests at 63.2% statement coverage, all 96 traceability-validator and 101
...
outputs, inventory, and all links across 168 Markdown files.

## Claim exact-SHA CI and receipt boundary

- Workflow: `repository-gate`.
- Run: [32783047837](https://github.com/jihadismail8/netbox-go/actions/runs/32783047837).
- Job `check`: [97609044839](https://github.com/jihadismail8/netbox-go/actions/runs/32783047837/job/97609044839).
- Branch: `work/cw1-v2-02-i5-rack-type-presence`.
- Head SHA: `25d6f10bab78b9b92c1682fe9b951cc4f2286ea8`.
- Run start/completion: `2026-08-24T22:06:58Z`–`2026-08-24T22:26:42Z`.
- Job start/completion: `2026-08-24T22:07:01Z`–`2026-08-24T22:26:41Z`.
- Conclusion: **success**; every setup, repository, post, and completion step
  succeeded, with no cancellation or non-success conclusion.

That exact-SHA result made only bounded I5 `evidence`. The claim commit
retains the exact two-path old/new mapping whose canonical JSON SHA-256 is
`e15cf464a5e5718e2864dc330b0e8c8b618d8bb32d971c4d8af8a03582b69645`.
...
concrete matrix, numeric-ID-only Manufacturer input, or rejected-RackType
durable side-effect checks. No local adapter/parity test substitutes for that
boundary, so strict REST differential proof, REST T2, and corresponding gRPC
T3 remain pending. The Vue unit/type boundary earns no browser T4.

Generic DRF numeric/string coercion, malformed UTF-8 behavior, byte-exact
generic diagnostics, empty PATCH, alternate Manufacturer input forms,
uniqueness, delete protection, list/get/delete/filter/search/order/pagination,
Rack propagation, and the 13-resource parent remain open. The locked frontend
dependency tree still reports three high-severity audit findings; dependency
changes are outside this packet, and no finding is represented as resolved.

## Gate disposition

- L0–L3: passed for the bounded RackType domain, application, REST, generated
  contract, gRPC parity regression, and Vue boundary on one unchanged tested
  source digest.
...
1. `docs/evidence/2026-08-24-core-workflow-v2-rack-type-scalar-presence-i5-v0.md`
   — new
...
are byte-identical at 3,043 entries and that the complete derived source diff
...
`89507d95d2743de7f97d64ca14cc43f6b834770b`. The evidence-only worktree
reconstructs the same bytes, so this is also its complete current manifest.

```json
{
  "schema_version": 2,
  "digest": "source-v2:sha256:a8325eaae703aa801ed587deefae7e8d08d9c9e0189c80ff7569da95c36d6f90",
  "files": 3043,
  "entries": [
    {
      "kind": "file",
    ...
    },
    {
      ...
      "path": "AGENTS.md",
      "size": 10893,
      "sha256": "sha256:934baa9067beed1d13725c56bf43bb8c60de57e6c2c5e51795e33aacae6547c8"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size": 9870,
      "sha256": "sha256:3c38d07b1912c6060161b2b9363720b82d0f4d84fc86f3e1f2069b1470f8b6fc"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size": 308752,
      "sha256": "sha256:41154793a4d7f4f9c3cfe729ab70d4fc354b80e6876059c792d1e0b822199a83"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size":