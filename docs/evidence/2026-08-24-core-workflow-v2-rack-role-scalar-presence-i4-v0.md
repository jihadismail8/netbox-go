# Core Workflow RackRole scalar write presence — 2026-08-24
...
- Result: **the bounded source candidate passed every required local,
  ...
  completion is retained below.
- Capability Profile: `core-workflow-v1`, still T1 and pre-publication.

This artifact proves only the four-field RackRole scalar create/PUT/PATCH
presence, defaults, and validation slice specified by the
[`CW1-V2-02-I4` packet](../increments/CW1-V2-02-I4.md). It does not complete
RackRole CRUD, the 13-resource parent, `CW1-V2-02`, V2, any compatibility
tier, the Capability Profile, the rewrite, deployment, or production
readiness.
...
- Entry revision:
  `1f0a7ccab6c20e6e5db529c6179312afc7c86a18`.
- Entry source digest:
  `source-v2:sha256:880a95bbde0f812e8a422344f7555b6b6d1ef527555c97537e451ad1049505fa`
  with 3,029 entries.
- Reviewed packet revision:
  `134c9cd01e3c04169c4dc9d9eb29b889865c2847`.
- Packet-acceptance revision:
  `76c151b9ce5e5b6a6d25ec6e9fbf14cb649be3f6`.
- Tested source candidate:
  `f1ef3d5e21b66a8e2f77bd380c09c81a8ef5dbfe`.
- Tested source digest:
  `source-v2:sha256:68db7a9835545d66ef9a651b9c4a000c91f3d834b5503a1b89e4a122275c3bc9`.
- Tested owned entries: 3,036.
- Canonical source-manifest output SHA-256:
  `91d7b6348731ec9c9ccddb31f3660d9dcf2b442e89867822fb12302f560d10b2`.
- Evidence-claim source digest: unchanged from the tested source digest, with
  the same 3,036 owned entries.
- Evidence-claim source-manifest output SHA-256: unchanged at
  `91d7b6348731ec9c9ccddb31f3660d9dcf2b442e89867822fb12302f560d10b2`.
- Complete derived source-manifest diff SHA-256:
  `5cd5668a6b4d2be88735d64db35bb9b2ecfb52815533bfca3c4685e46eb33c2d`.
- Go: `go1.26.0 linux/amd64`.
- Node.js: `v24.18.0`.
- npm: `11.16.0`.

The packet was accepted before any behavior edit. The preserved
pre-main-rebase I1 stash was not applied, modified, or dropped. The exact
feature branch was pushed normally; `main` was not changed.

## Implemented boundary

- Operation-specific RackRole metadata generates distinct `RackRoleCreate`,
  `RackRoleReplace`, and `RackRoleUpdate` request schemas while the response
  keeps only its two baseline timestamps nullable.
- POST requires `name` and `slug`, defaults omitted `color` to `9e9e9e`, and
  defaults omitted `description` to `""`. PUT requires `name` and `slug` but
  preserves omitted `color` and `description`. PATCH preserves every omitted
  scalar.
- Explicit null is rejected for all four fields. Blank `name`, `slug`, and
  `color` are rejected; blank `description` durably clears it. All four fields
  are trimmed. Name and slug are limited to 100 Unicode code points,
  description to 200, slug retains the shared pinned ASCII grammar, and color
  must be exactly six lowercase hexadecimal digits.
- The typed command/domain boundary covers the complete 48 primary
  operation-by-field-by-state cells, preserves valid sibling intent during
  mixed validation, orders diagnostics as `name`, `slug`, `color`, then
  `description`, and previews patches without mutating state.
- Every validation-rejected mutation makes zero repository create/update
  calls and leaves complete RackRole state, timestamps, object-change count,
  RackRole count, related rack count, and global change count unchanged. A
  forced recorder failure follows an attempted repository update, then the
  application-owned transaction rolls back the durable row, timestamps,
  related-rack count, and object-change record. Every successful public
  POST/PUT/PATCH persists the intended state and exactly one object change in
  that transaction.
- REST retains omitted, explicit-null, blank, and concrete scalar intent.
  Supported gRPC FieldMask paths with absent optional scalar values map to
  explicit null intent; unsupported paths retain fail-closed `update_mask`
  validation. Optional proto scalars still cannot distinguish POST/PUT null,
  and this increment does not redesign or regenerate protobufs.
- Real PostgreSQL reloads the four scalar columns, proves timestamps,
  RackRole/global row and change counts, preserves a real related-rack count,
  and proves rollback for presence/domain and forced recorder failures.
- Vue retains nullable response timestamps, required name/slug and optional
  color/description form metadata, the `9e9e9e` create default,
  omission/blank/concrete create serialization, and a private hydrated scalar
  snapshot so unchanged PATCH fields are omitted while an explicit
  description clear sends `""`.

RackRole uniqueness, delete protection, list/get/filter/order/pagination
behavior, permission expansion, empty-PATCH product semantics, relationships,
full CRUD, and all other resources remain outside this increment.

## Changed tested-source files

The tested candidate differs from the accepted I3 entry revision across
exactly 30 regular mode-`100644` paths:
...
5. `docs/STATUS.md`
6. `docs/TESTING.md`
7. `docs/increments/CW1-V2-02-I3.md` — the opening packet records the already-green I3 receipt
8. `docs/increments/CW1-V2-02-I4.md` — new
9. `docs/increments/README.md`
10. `netbox-backend/api/openapi/netbox-go-v1.yaml`
11. `netbox-backend/internal/adapters/grpc/workflow/organization_typed_mapper.go`
12. `netbox-backend/internal/adapters/grpc/workflow/organization_typed_server_test.go`
13. `netbox-backend/internal/adapters/grpc/workflow/rack_role_typed_mapper_test.go` — new
14. `netbox-backend/internal/adapters/postgres/dcim/rack_role_service_real_postgres_test.go` — new
15. `netbox-backend/internal/adapters/rest/netbox/workflow/rack_role_scalar_presence_test.go` — new
16. `netbox-backend/internal/application/dcim/organization_service_test.go`
17. `netbox-backend/internal/application/dcim/rack_role_commands.go`
18. `netbox-backend/internal/application/dcim/rack_role_commands_test.go` — new
19. `netbox-backend/internal/application/dcim/rack_role_service.go`
20. `netbox-backend/internal/application/dcim/rack_role_service_presence_test.go` — new
21. `netbox-backend/internal/domain/dcim/rack_role.go`
22. `netbox-backend/internal/domain/dcim/rack_role_test.go`
23. `netbox-backend/test/parity/rack_role_scalar_presence_test.go` — new
24. `netbox-frontend/src/features/core/adapters.test.ts`
25. `netbox-frontend/src/features/core/adapters.ts`
26. `netbox-frontend/src/features/core/resources.ts`
27. `netbox-frontend/src/router/models/core-profile.test.ts`
28. `scripts/validate_capability_profile.mjs`
29. `scripts/validate_capability_profile.test.mjs`
30. `scripts/validate_openapi.mjs`

The entry-to-candidate delta contains 3,315 insertions and 239 deletions,
...
```text
TestRackRoleScalarNormalizationContract
TestRackRoleScalarCommandPresenceMatrix
TestReplaceRackRolePreservesOmittedState
TestRackRoleScalarValidationLeavesStateUnchanged
TestRackRoleRESTScalarPresenceMatrix
TestRackRoleGRPCScalarPresenceMatrix
TestPostgresRackRoleScalarPresenceDurability
TestRackRoleScalarPresenceRESTGRPCParity
```

They span the domain, application, REST, gRPC, PostgreSQL, and parity packages.
Independent backend, PostgreSQL/parity, and contract/Vue reviews found no
remaining blocker after the parity matrix was expanded to cover every
transport operation/state cell. No test, comparator, normalizer, security
...
13.619 seconds without race instrumentation and 172.666 seconds with it.

The focused durable PostgreSQL test completed in 0.266 seconds without a
skip. The final exact committed full L4 replay passed with these package
timings:

| Package                                | Result  |
| -------------------------------------- | ------: |
| `internal/database`                    |  2.336s |
| `internal/adapters/postgres/bootstrap` |  0.042s |
| `internal/adapters/postgres/dcim`      |  1.983s |
| `internal/adapters/postgres/dcim/row`  |  0.332s |
| `internal/adapters/postgres/ipam`      |  1.749s |
| `internal/adapters/postgres/ipam/row`  |  0.390s |
| `internal/adapters/postgres/changelog` |  0.003s |
| `internal/application/dcim`            |  0.004s |
| `internal/application/ipam`            |  0.073s |
| `internal/application/identity`        | 35.728s |
| `cmd/netbox_go_admin`                  |  6.473s |

The run used exact disposable database `netbox_i4_20260824` in the existing
test PostgreSQL container with the shared test owner; no disposable role was
created. No credential or complete DSN is retained. Before the final drop the
database had zero connections. It was dropped; final database,
same-named-role, and connection counts were `0|0|0`. The shared owner was
untouched. The disposable database is not recoverable.

Initial sandbox-only Node/Git/esbuild, loopback-socket, and PostgreSQL-socket
attempts were denied with `EPERM`. The identical approved isolated commands
passed. One initial root-gate invocation supplied a Node-only PATH and could
not find Go; the corrected pinned Node-plus-Go invocation passed. These
environment-only failures were not behavioral failures and no gate was
weakened.
...
- all 75 adversarial capability-profile and generated-OpenAPI validator tests;
...
- generated-document consistency and OpenAPI validation at 33 paths and 86
  operations;
- 29 focused Vue adapter/form tests and both production and test typechecks;
- the complete frontend gate: 29 files, 149 tests, and 62.81% statement
  coverage, plus formatting, lint, typechecking, and production build; and
- syntax validation of the compatibility driver.
...
env \
  ...
  GOLANGCI_LINT_CACHE=/tmp/netbox-go-i4-rack-role-golangci-cache \
  npm_config_cache=/tmp/netbox-go-npm-cache \
  make check
```

It exited 0. Results included:

- backend lint: zero issues;
- backend coverage: 12.7221% (37,053/291,250), above the retained 12.4001%
  (36,013/290,425) floor, with 54 measured packages and one reviewed
  exclusion;
- all capability, traceability, OpenAPI, generated-contract, and frontend
  results above; and
- all local links valid across 165 Markdown files.

`git diff --check`, source-digest, complete source-manifest, generated-output,
and documentation checks also passed. The tested source digest and manifest
were unchanged across focused, affected, race, PostgreSQL, L4, contract,
frontend, backend, whole-repository, commit, and push boundaries.

## Independent candidate CI

- Workflow: `repository-gate`.
- Run: [32754023762](https://github.com/jihadismail8/netbox-go/actions/runs/32754023762).
- Job `check`: [97517303191](https://github.com/jihadismail8/netbox-go/actions/runs/32754023762/job/97517303191).
- Branch: `work/cw1-v2-02-i4-rack-role-presence`.
- Head SHA: `f1ef3d5e21b66a8e2f77bd380c09c81a8ef5dbfe`.
- Run start/completion: `2026-08-24T16:59:30Z`–`2026-08-24T17:20:46Z`.
- Job start/completion: `2026-08-24T16:59:34Z`–`2026-08-24T17:20:46Z`.
...
`f1ef3d5e21b66a8e2f77bd380c09c81a8ef5dbfe`, not a different merge revision.

## Digest-excluded claim local gate

The evidence-only worktree changes only the two source-v2-excluded paths
listed below. Its source digest, 3,036-entry manifest, and canonical manifest
SHA-256 therefore remain byte-identical to the tested candidate. The complete
derived source-manifest diff is empty.

The exact evidence-only local replay used the pinned `make check` command
shown above from `2026-08-24T17:38:20Z` through
`2026-08-24T17:43:42Z` and exited 0. It retained backend coverage at 12.7221%
(37,053/291,250), 54 measured packages and one reviewed exclusion, all 29
frontend files and 149 tests at 62.81% statement coverage, all 96
traceability-validator and 75 capability/OpenAPI adversarial tests, the
...
syntax validation, but its current broad matrix does not prove this
increment's complete four-field POST/PUT/PATCH omitted/null/blank/concrete
matrix or rejected-RackRole durable side-effect checks. No local
adapter/parity test substitutes for that boundary, so strict REST differential
proof, REST T2, and corresponding gRPC T3 remain pending. The Vue unit/type
boundary earns no browser T4.

Generic DRF numeric-to-string coercion, malformed UTF-8 parser behavior, and
byte-exact generic character-field diagnostics remain deferred exactly as the
packet records. RackRole uniqueness, delete protection,
list/get/filter/order/pagination behavior, permission expansion, empty-PATCH
product semantics, full CRUD, and the 13-resource parent remain open.

## Gate disposition

- L0–L3: passed for the bounded RackRole domain, application, REST, generated
...
`f1ef3d5e21b66a8e2f77bd380c09c81a8ef5dbfe`. The evidence-only worktree
reconstructs the same bytes, so this is also its complete current manifest.

```json
{
  "schema_version": 2,
  "digest": "source-v2:sha256:68db7a9835545d66ef9a651b9c4a000c91f3d834b5503a1b89e4a122275c3bc9",
  "files": 3036,
  "entries": [
    {
      "kind": "file",
    ...
    },
    {
      ...
      "path": "AGENTS.md",
      "size": 10004,
      "sha256": "sha256:0500b1940d84dcdd899333d956f495777db3925dda43425d0b0a69f0123791e7"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size": 9214,
      "sha256": "sha256:7e2922f7e5410e68b9b0f2fc3b0ef1f855f2be2a4b45bf9131066bfe5e594593"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size": 308047,
      "sha256": "sha256:30b57206f82e2c343a36908e1634de2d027fd564b8dddab4ede65698968c6e3f"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size": 83872,
      "sha256": "sha256:d79a423337556cbd76f1e6ecc14409da25d519742c9039594e592808e1e06559"
    },
    {
      "kind": "file",
      "mode": "100644",
      "path": "docs/STATUS.md",
      "size": 27442,
      "sha256": "sha256:8d49dd8883cd829c59d710e0d5149a6141bc222f6045314c0c0a38b8d019dee9"
    },
    {
      "kind": "file",
      "mode": "100644",
      ...
      "size": 18074,
      "sha256": "sha256:3cba933ab8ed8d4b508340e36857a60085e13cf1b51a31b01d00e8d4ec6eb8f9"
    },
    {
      "kind": "file",
      "mode": "100644",
    ...
    },
    {
      ...
      "size":