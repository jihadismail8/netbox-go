...
| 3    | Traceable differential/parity proof | `CW1-V2-*`, `CW1-V4-*`                  | V2-01/I1–I9 done; parent/T2/T3 open                                     |
...
closeout receipt exact-SHA CI are green, so I3 is effectively `done`. No
retained pinned differential accompanies this bounded result, so T2/T3 remain
...
`2026-08-24T18:51:56Z`; its owner-accepted closeout claim and excluded receipt
passed exact-SHA CI, so I4 is effectively `done`. No retained differential
accompanies the result, so T2/T3
...
[`CW1-V2-02-I5`](increments/CW1-V2-02-I5.md) has exact tested candidate
...
with 3,043 entries. It owns only the ten declared RackType writable fields
across create/PUT/PATCH operation metadata, typed shared semantics,
...
[`CW1-V2-02-I6`](increments/CW1-V2-02-I6.md) has exact tested candidate
...
with 3,050 entries. It owns only DeviceRole `parent`, `name`, `slug`, `color`,
`vm_role`, `description`, and `comments` create/PUT/PATCH presence across
operation-specific contracts, shared typed behavior, PostgreSQL durability,
...
`2026-08-25T08:13:40Z`; its owner-accepted closeout claim and excluded receipt
passed exact-SHA CI, so I6 is effectively `done`. No retained differential
accompanies the result, so T2/T3 remain unearned. DeviceRole hierarchy,
uniqueness, deletion, list/query,
full CRUD, every tier/consumer, this parent, the profile, and every later
child remain open.

[`CW1-V2-02-I7`](increments/CW1-V2-02-I7.md) has exact tested candidate
...
with 3,057 entries. It owns only DeviceType `manufacturer`, `model`, `slug`,
`part_number`, `u_height`, `exclude_from_utilization`, `is_full_depth`,
`airflow`, `description`, and `comments` common create/PUT/PATCH presence
across operation-specific contracts, shared typed semantics, PostgreSQL
durability, and Vue dirty-field handling. Its candidate, evidence-claim, and
pre-acceptance receipt exact-SHA CI boundaries are green. The project owner's
acceptance of only this bounded result was recorded at
`2026-08-25T14:12:13Z`; its owner-accepted closeout claim and excluded receipt
passed exact-SHA CI, so I7 is effectively `done`. No retained differential
accompanies the result, so T2/T3
remain unearned. DeviceType uniqueness, positioned-Device height transitions,
deletion/cascades, list/query behavior, full CRUD, every tier/consumer, this
parent, the profile, and every later child remain open.

[`CW1-V2-02-I8`](increments/CW1-V2-02-I8.md) has exact tested candidate
...
with 3,064 entries. It owns only InterfaceTemplate `device_type`, `name`,
`label`, `type`, `enabled`, `mgmt_only`, and `description` common
create/PUT/PATCH presence across operation-specific contracts, shared typed
semantics, PostgreSQL durability, and Vue dirty-field handling. Its candidate,
evidence-claim, and pre-acceptance receipt exact-SHA CI boundaries are green.
The project owner's acceptance of only this bounded result was recorded at
`2026-08-26T02:27:06Z`; its owner-accepted closeout claim and excluded receipt
passed exact-SHA CI, so I8 is effectively `done`. No retained differential
accompanies the result, so T2/T3
remain unearned. Existing DeviceType-owner containment is exercised without
closing owner immutability. InterfaceTemplate uniqueness, ModuleType
ownership, bridge behavior, Device instantiation/snapshot/rollback,
...
passed exact-SHA CI, so bounded I9 is effectively `done`. No retained
differential accompanies the result, so T2/T3 remain unearned. Rack
uniqueness, mounted-device and placement rules,
...
| `CW1-V2-02`         | `in-progress` | Complete remaining common CRUD children             | Accepted V2-01 and current `CW1-G00`   |
...
| `CW1-V3-01`         | `in-progress` | Make HTTP/gRPC readiness dependency-aware           | `CW1-G00`                              |
...
resolution, gRPC streams, and aggregate all-transport coverage remain open.
...
- [`CW1-V2-02-I2`](increments/CW1-V2-02-I2.md) owns only Site `name`, `slug`,
...
- [`CW1-V2-02-I3`](increments/CW1-V2-02-I3.md) owns only Manufacturer
  ...
  pre-acceptance receipt, owner-accepted closeout claim, and excluded closeout
  receipt exact-SHA CI are green. The project owner accepted only this bounded
  result at `2026-08-24T10:28:34Z`; I3 is effectively `done`. No retained
  pinned differential accompanies this result, so T2/T3 remain unearned.
...
- [`CW1-V2-02-I4`](increments/CW1-V2-02-I4.md) owns only RackRole `name`,
  ...
  `2026-08-24T18:51:56Z`; its closeout claim and excluded receipt passed
  exact-SHA CI, so I4 is effectively `done`. No
...
- [`CW1-V2-02-I5`](increments/CW1-V2-02-I5.md) owns only RackType
  `manufacturer`, `model`, `slug`, `form_factor`, `width`, `u_height`,
  `starting_unit`, `desc_units`, `description`, and `comments`
...
- [`CW1-V2-02-I6`](increments/CW1-V2-02-I6.md) owns only DeviceRole `parent`,
  ...
  Its exact candidate, evidence claim, pre-acceptance receipt,
  owner-accepted closeout claim, and excluded receipt passed exact-SHA CI. The
  project owner accepted only this bounded result at
  `2026-08-25T08:13:40Z`; bounded I6 is effectively `done`.

- [`CW1-V2-02-I7`](increments/CW1-V2-02-I7.md) owns only DeviceType
  `manufacturer`, `model`, `slug`, `part_number`, `u_height`,
  `exclude_from_utilization`, `is_full_depth`, `airflow`, `description`, and
  `comments` common create/PUT/PATCH presence, operation-specific generated
  API contracts, matching REST/gRPC semantics, PostgreSQL durability, Vue
  dirty-field serialization/validation, and eight fixed regressions.
  DeviceType uniqueness, positioned-Device height transitions, deletion and
  template cascades, list/query behavior, full CRUD, tiers, this parent, and
  every traceability consumer remain open. Its exact candidate, evidence
  claim, and pre-acceptance receipt passed exact-SHA CI. The project owner
  accepted only this bounded result at `2026-08-25T14:12:13Z`; its
  owner-accepted closeout claim and excluded receipt passed exact-SHA CI, so
  bounded I7 is effectively `done`.

- [`CW1-V2-02-I8`](increments/CW1-V2-02-I8.md) owns only InterfaceTemplate
  `device_type`, `name`, `label`, `type`, `enabled`, `mgmt_only`, and
  `description` common create/PUT/PATCH presence, operation-specific generated
  API contracts, matching REST/gRPC semantics, PostgreSQL durability, Vue
  dirty-field serialization/validation, and eight fixed regressions.
  Existing DeviceType-owner containment is exercised without closing owner
  immutability. InterfaceTemplate uniqueness, ModuleType ownership,
  bridge behavior, Device instantiation/snapshot/rollback,
  non-retroactivity, deletion, list/query behavior, full CRUD, tiers, this
  parent, and every traceability consumer remain open. Its exact candidate,
  evidence claim, and pre-acceptance receipt passed exact-SHA CI. The project
  owner accepted only this bounded result at `2026-08-26T02:27:06Z`; its
  owner-accepted closeout claim and excluded receipt passed exact-SHA CI, so
  bounded I8 is effectively `done`.

- [`CW1-V2-02-I9`](increments/CW1-V2-02-I9.md) owns only Rack `site`, `name`,
  `facility_id`, `rack_type`, `status`, `role`, `serial`, `asset_tag`,
  `form_factor`, `width`, `u_height`, `starting_unit`, `desc_units`, `airflow`,
  `description`, and `comments` common create/PUT/PATCH presence,
  operation-specific generated API contracts, matching REST/gRPC semantics,
  direct-save RackType copy precedence, PostgreSQL durability, Vue dirty-field
  ...
  claim, and pre-acceptance receipt passed exact-SHA CI. The project owner
  accepted only this bounded result at `2026-08-26T11:01:23Z`; its
  owner-accepted closeout claim and excluded two-path receipt passed exact-SHA
  CI, so bounded I9 is effectively `done`. No later child is claimed.
