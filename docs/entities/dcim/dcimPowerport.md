...
> Module: `dcim` | Table: `dcim_powerport` | Python class: `PowerPort` | File: `dcim/models/device_components.py`

**Inheritance:** `ModularComponentModel <- CabledObjectModel <- PathEndpoint <- TrackingModelMixin`

**REST URL:** `/api/dcim/power-ports/`

## Implementation Status

- [ ] Go model (`internal/model/dcimPowerport.go`)
- [ ] GORM mapping verified (column names, types, constraints)
- [ ] Column whitelist complete
- [ ] Proto definition (`api/netbox_go/v1/dcimPowerport.proto`)
- [ ] Proto generated code (`.pb.go`, `_grpc_pb.go`, `.pb.validate.go`)
- [ ] DAO layer (`internal/dao/dcimPowerport.go`)
- [ ] DAO unit tests (`internal/dao/dcimPowerport_test.go`)
- [ ] Cache layer (`internal/cache/dcimPowerport.go`)
- [ ] Cache unit tests (`internal/cache/dcimPowerport_test.go`)
- [ ] Service layer (`internal/service/dcimPowerport.go`)
- [ ] Service unit tests (`internal/service/dcimPowerport_test.go`)
- [ ] Handler layer (`internal/handler/dcimPowerport.go`)
- [ ] HTTP routes registered
- [ ] Error codes defined (`internal/ecode/`)
- [ ] REST URL matches NetBox convention (`/api/dcim/power-ports/`)
...
|-------|---------------|-----------|------|--------------|
| `device` | [Device](./dcimDevice.md) | `CASCADE` | No | `powerports` |
...
| `module` | [Module](./dcimModule.md) | `CASCADE` | Yes | `powerports` |
...
| Field | Type | Notes |
|-------|------|-------|
| `type` | CharField(50) | choices=PowerPortTypeChoices; null=True |
| `maximum_draw` | PositiveIntegerField | null=True; min=1 (watts) |
| `allocated_draw` | PositiveIntegerField | null=True; min=1 (watts) |
...
- [PowerOutlet](./dcimPoweroutlet.md) via `power_port` (FK, related_name=`poweroutlets`)
...
- **Go model file:** `internal/model/dcimPowerport.go`
- **Proto file:** `api/netbox_go/v1/dcimPowerport.proto`
- Validation: `allocated_draw` cannot exceed `maximum_draw`
- Complex business logic: `get_downstream_powerports()` traces power through child PowerOutlets
- `get_power_draw()` calculates aggregate power utilization including three-phase leg calculations
## CRUD Behavior
