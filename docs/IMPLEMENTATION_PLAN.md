...
| 5 — identity/RBAC                   | Users, groups, memberships, direct/group/global/object grants, sessions, tokens, REST/gRPC adapters, bearer auth, administrator CLI, retained trusted-origin CORS, accepted token lookup/touch, token-transport, and browser-session/CSRF evidence are present. The exact I4 tested candidate passed its local, real-PostgreSQL, feature-CI, and main-CI boundaries; its claim-only evidence transition is conditional on the current attestation revision's CI. | Retain the I4 attestation receipt and obtain project-owner review; then complete Origin/Referer/masking, password-policy, throttle/proxy/streaming, RBAC, CLI, administration, and secret evidence. |
...
- Browser sessions use `HttpOnly`, `Secure` in production, `SameSite` cookies plus CSRF protection; JavaScript stores no credential. The declared password-change policy rotates a browser-session-origin request into exactly one fresh current session while revoking every old browser session. REST Token and gRPC bearer origins revoke every browser session without issuing browser credentials; API tokens remain governed by their independent lifecycle. I4's exact tested candidate passed its accepted-I3-based red-first, local, real-PostgreSQL, feature-CI, and main-CI boundaries. Its bounded `evidence` state is conditional on the exact claim-only attestation revision passing CI; the receipt and project-owner review then remain.
