...
| 5 — identity/RBAC                   | Users, groups, memberships, direct/group/global/object grants, sessions, tokens, REST/gRPC adapters, bearer auth, administrator CLI, retained trusted-origin CORS, accepted token lookup/touch evidence, and bounded token-transport evidence are present. | Review I2; complete session/CSRF/throttle/proxy/streaming, RBAC, CLI, administration, and secret evidence.                                              |
