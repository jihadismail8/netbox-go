...
| 5 — identity/RBAC                   | Users, groups, memberships, direct/group/global/object grants, sessions, tokens, REST/gRPC adapters, bearer auth, administrator CLI, retained trusted-origin CORS, accepted token lookup/touch and token-transport evidence, and accepted bounded browser-session/CSRF evidence are present. | Complete password-session, Origin/Referer/masking, throttle/proxy/streaming, RBAC, CLI, administration, and secret evidence.                            |
