...
| 5 — identity/RBAC                   | Users, groups, memberships, direct/group/global/object grants, sessions, tokens, REST/gRPC adapters, bearer auth, administrator CLI, retained trusted-origin CORS, accepted token lookup/touch, token-transport, and browser-session/CSRF evidence, plus an active rebased I4 candidate with a green pre-governance local source-candidate ladder, are present. | Re-run the governance-inclusive exact-byte gates, commit/push the candidate, and retain I4; then complete Origin/Referer/masking, password-policy, throttle/proxy/streaming, RBAC, CLI, administration, and secret evidence. |
