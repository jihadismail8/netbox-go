...
| 5 — identity/RBAC                   | Users, groups, memberships, direct/group/global/object grants, sessions, tokens, REST/gRPC adapters, bearer auth, administrator CLI, retained trusted-origin CORS, and bounded token lookup/touch evidence are present.                               | Complete the remaining cross-transport credential, RBAC, CSRF/throttling, session, CLI, and secret matrix.                                              |
