...
| Diagnostics       | The canonical router keeps `/health` process-only and evaluates `/ready` through a one-second request-time PostgreSQL checker. Standard empty-service gRPC Health Check uses the same injected checker; named services fail NotFound and Watch is explicitly unsupported. Both listeners receive one checker from startup composition. No dedicated `GET /ping` is registered; frozen ADR 0004 legacy wiring retains only its runtime-disabled historical route. | The bounded dependency-aware implementation and deterministic focused/race regressions are present. Exact candidate/repository evidence and V3-04 real deployment loss/recovery remain required; no production-operability claim is made. |
...
post-wrapper-cleanup source. V1-V5 still require implementation/evidence work—
remaining identity policy, readiness candidate retention and real deployment
loss/recovery, rule traceability, missing oracle/parity cases, and missing browser outcomes—before their external evidence can be
